...
const ffmpeg_bullder_1 = require("./ffmpeg.bullder");
...
class FfmpegExecutor extends command_executor_1.CommandExecutor {
    ...
    build({ width, height, path, name }) {
        ...
        const args = (new ffmpeg_bullder_1.FfmpegBuilder()).input(path).setVideoSize(width, height).output(output);
    ...
    processStream(stream, logger) {
        const handler = new stream_handler_1.StreamLogger(logger);
